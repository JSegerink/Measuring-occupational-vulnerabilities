...
############################IN MICROHEIGHTS################################
#################EXAMPLE 'D' in Prison_Czech_at_Repy_1850_steg ############
###########################################################################
...
#the spelling variations of 'dienstmagd'
var_occ = filter(cln_c_db, databaseID == '7' & 
                   occ == 'D' | occ == 'd' | occ == 'Dienst' |
                   occ == 'dienstm' | occ == 'Dienstm' | 
                   occ == 'Dienstmädchen' | occ == 'Dienstmagd'
)


freq <- table(var_occ$occ, var_occ$databaseID)
view(freq)
